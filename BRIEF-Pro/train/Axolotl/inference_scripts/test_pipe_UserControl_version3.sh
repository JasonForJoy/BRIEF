...
ALLOW_NUM_GPU="2"
SETTINGS=(
    # "My_Axolotl_Finetuned_MuSiqQue_x4_UserControl_version3_Final_5sentences"
    "My_Axolotl_Finetuned_MuSiqQue_x4_UserControl_version3_Final_Auto"
    )
MERGE_PATHS=(
    ...
    "/data2/junyizhang/BRIEF_train_eval_Code/Axolotl/outputs/My_finetune_MuSiqQue_x4_UserControl_version3_Final_merged"
    )
...
for i in "${!SETTINGS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="${ALLOW_GPUS}" python /data2/junyizhang/BRIEF_train_eval_Code/Axolotl/inference_scripts/vllm_compression_longbench_UserControl_version3.py --name "${SETTING}" --model_path "${MERGE_PATH}" --parallel_size "${ALLOW_NUM_GPU}"
