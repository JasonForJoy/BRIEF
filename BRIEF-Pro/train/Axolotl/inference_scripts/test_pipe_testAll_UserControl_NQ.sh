...
ALLOW_NUM_GPU="1"
SETTINGS=(
    # "My_Axolotl_Finetuned_NQ_UserControl_checkpoint378_20sentences_testAll"
    "My_Axolotl_Finetuned_NQ_noCleanFuther_checkpoint1214_Auto_testAll"
    )
MERGE_PATHS=(
    ...
    "/data2/junyizhang/BRIEF_train_eval_Code/Axolotl/outputs/My_finetune_NQ_noCleanFuther_merged_checkpoint1214"
    )
...
for i in "${!SETTINGS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="${ALLOW_GPUS}" python /data2/junyizhang/BRIEF_train_eval_Code/Axolotl/inference_scripts/vllm_compression_longbench_testAll_UserControl_NQ.py --name "${SETTING}" --model_path "${MERGE_PATH}" --parallel_size "${ALLOW_NUM_GPU}"
