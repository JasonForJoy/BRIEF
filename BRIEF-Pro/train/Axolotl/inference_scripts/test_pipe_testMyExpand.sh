...
for i in "${!SETTINGS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="${ALLOW_GPUS}" python /data2/junyizhang/BRIEF_train_eval_Code/Axolotl/inference_scripts/vllm_compression_longbench_MyExpand_UserControl.py --name "${SETTING}" --model_path "${MERGE_PATH}" --parallel_size "${ALLOW_NUM_GPU}"
    ...
    CUDA_VISIBLE_DEVICES="${ALLOW_GPUS}" python /data2/junyizhang/BRIEF_train_eval_Code/LongBench/LongBench/pred_testMyExpand.py --model "llama3.1-8b-instruct-128k" --e --setting "${SETTING}"
    ...
    python /data2/junyizhang/BRIEF_train_eval_Code/LongBench/LongBench/eval_MyExpand.py --model "llama3.1-8b-instruct-128k_${SETTING}"
