...
def do_inference(
    *,
    ...
    while True:
        ...
        print("=" * 40)
        print(f"Literal prompt length is:{len(prompt)}")
        print(f"Prompt: {prompt}")
        print("=" * 40)
        batch = tokenizer(prompt, return_tensors="pt", add_special_tokens=True)
        print("=" * 40)
        print(f"Begin generation! Input length is:{len(batch['input_ids'][0])}")
        print("=" * 40)
        model.eval()
        with torch.no_grad():
        ...
        print("=" * 40)
        print(f"all output length is: {len(generated['sequences'][0])}")
        print("=" * 40)
