...
def compute_weighted_loss(
    ...
    num_items_in_batch: int | None = None,   # 由 Trainer 传入，可忽略
    **kwargs,
):
    """
    让同一条 label 中连续的可训练片段（指令句 + 摘要句 …）各占相同总权重
    e.g. 2 段 -> 每段占 0.5；3 段 -> 每段占 0.333...
    """
    ...
    # np.savetxt('./labels.txt', labels.cpu().numpy(), fmt='%d')
    # np.savetxt('./input_ids.txt', inputs["input_ids"].cpu().numpy(), fmt='%d')
    ...
    for i in range(labels.size(0)):
        ...
        for k in range(0, labels[i].size(0)):
    ...
    # np.savetxt('./labels_AfterMask.txt', labels.cpu().numpy(), fmt='%d')
    ...
    # causal LM: shift so that tokens <n> predict <n+1>
    ...
    # print("loss_per_tok.shape", loss_per_tok.shape)
    ...
    for i in range(labels.size(0)):
        row   = labels[i]
        mask  = row != -100                       # 可训练 token
        gids  = torch.zeros_like(row).int()       # 片段编号

        # print("gids.shape", gids.shape)
        ...
        # np.savetxt('./gids.txt', gids.cpu().numpy(), fmt='%d')
        ...
        weights[i, mask] = 1.0 / counts[gids[mask]]
    ...
    # np.savetxt('./weights.txt', weights.cpu().numpy(), fmt='%.4f')
    # print("#################################################################")

    # -------------------------------------------------------------------------------------------------------

    mask_loss_predict_K = (weights == 0.5).float() * 0.5
    # print(mask_loss_predict_K.sum())

    # loss_predict_K = (loss_per_tok * mask_loss_predict_K).sum() / mask_loss_predict_K.sum()
    loss_predict_K = ( (loss_per_tok * mask_loss_predict_K).sum() / sum_weights ) * 2
    


    print("sum_weights:", sum_weights)
    print(f"loss_predict_K: {loss_predict_K}  |  mask_loss_predict_K.sum(): {mask_loss_predict_K.sum()}")


    # -------------------------------------------------------------------------------------------------------

    # loss = (loss_per_tok * weights).sum() / weights.sum()
    
    # loss = ( (loss_per_tok * weights).sum() / weights.sum() ) * 2

    loss = ( (loss_per_tok * weights).sum() / sum_weights ) * 2


    loss_answer = loss - loss_predict_K
    print(f"loss_answer: {loss_answer}")
    ...
    # -----------------------------------------------------------
    # 先把张量转成纯 Python 数值(.item())，这样 WandB 才能序列化
    # 用 trainer 的 global_step 作为 step。 没有也可不写 step
    if wandb.run is not None:                          # 训练脚本里已 wandb.init()
        wandb.log(
            {
                "train/loss_predict_K": loss_predict_K.detach().cpu().item(),
                "train/loss_answer"  : loss_answer.detach().cpu().item(),
            },
            step=self.state.global_step                # Trainer 里随手可用
            # commit=False  # 如想和后面别的 log 合并，可加上
        )
    # -----------------------------------------------------------
    ...
    if not skip_estimates and cfg.sample_packing:
        ...
        if cfg.sample_packing_eff_est:
            ...
            if cfg.flash_attention and not cfg.multipack_real_batches:
                # raise ValueError("Here!")     # 代码会执行此处，强制将sampler_batch_size 设置为1，而不是设置为micro_batch_size!
...
def setup_trainer(
    ...
    import types
    from axolotl.utils.trainer import compute_weighted_loss  # 就是上面写的那个
    trainer.compute_loss = types.MethodType(compute_weighted_loss, trainer)
