...
def main():
    ...
    args = parse_args()
    print(f" Save at: ../FLOPS/{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}_summary_flop.log")
    with open(f"../FLOPS/{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}_summary_flop.log","w") as f:
        f.write("test")
    ...
    with accelerator.split_between_processes(data) as prompts:
        ...
        for prompts_tokenized in tqdm(prompt_batches):
            with accelerator.profile() as prof:
                ...
                outputs_tokenized=model.generate(input_ids, attention_mask=attention_mask, max_new_tokens=args.max_new_tokens,num_beams = args.num_beams)


                # count and decode gen. tokens 
                outputs=tokenizer.batch_decode(outputs_tokenized,skip_special_tokens=True)
                
            profile_string = prof.key_averages().table(sort_by="flops", row_limit=4)
                

            aten_mm  = float(profile_string.split("\n")[3].split()[-1])
            aten_bmm = float(profile_string.split("\n")[4].split()[-1])
            aten_mul = float(profile_string.split("\n")[5].split()[-1])
            aten_add = float(profile_string.split("\n")[6].split()[-1])
            ...
            profile.append([aten_mm,aten_bmm,aten_mul,aten_add])
    ...
    profile_gathered=gather_object(profile)
    ...
    if accelerator.is_main_process:
        ...
        with open(f"../FLOPS/{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}_summary_flop.json","w") as f:
            json.dump(profile_gathered,f,indent=4)
