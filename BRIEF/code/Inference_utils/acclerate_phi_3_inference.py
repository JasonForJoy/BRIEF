...
from prompt import prompt,general_prompt,flan_fewshot_tqa,flan_fewshot_hotpot,flan_fewshot_nq,flan_fewshot_musique,job_description
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(model_name, device_map={"": accelerator.process_index},trust_remote_code=True)
    ...
    def prepare_prompts(prompts, tokenizer, batch_size=16):
        ...
        for prompt_batch in batches:
            ...
            p_prompt_batch = [apply_chat_template(get_final_prompt(fs=few_shot_string,doc=example['Proposition'],ques=example['Question'])) for example in prompt_batch]
            # p_prompt_batch = [
    ...
    with accelerator.split_between_processes(data) as prompts:
        ...
        for prompts_tokenized in tqdm(prompt_batches):
            ...
            # with accelerator.profile() as prof: 
            outputs_tokenized=model.generate(input_ids, attention_mask=attention_mask, max_new_tokens=512,temperature=0,top_p=0.95,eos_token_id=[tokenizer.eos_token_id, tokenizer.convert_tokens_to_ids("<|end|>")])
    ...
    if accelerator.is_main_process:
        ...
        # with open("../FLOPS/top5_flop_batch_1.log","w") as f:
