...
os.environ["WANDB_PROJECT"] = "Test-T5"  # name your W&B project
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The name of the column in the datasets containing the questions (for summarization).",
    )
    parser.add_argument(
        "--use_slow_tokenizer",
        action="store_true",
    ...
    if args.dataset_name is None and args.train_file is None and args.validation_file is None:
        ...
        if args.train_file is not None:
            ...
            assert extension in ["csv", "json","jsonl"], f"`train_file` should be a csv or a json file, but got {extension}."
...
def main():
    args = parse_args()
    args.output_dir = '{}-{}-{}'.format(args.output_dir, args.model_name_or_path.replace("/", "-"),
    ...
    if args.cache_dir:
        if args.config_name:
            config = AutoConfig.from_pretrained(args.config_name, trust_remote_code=args.trust_remote_code,cache_dir=args.cache_dir)
    ...
    assert args.question_column is not None
    ...
    def preprocess_summary_function(split,examples):
        ...
        for i in range(len(examples[question_column])):
            ...
            # questions.append(examples[question_column][i])
        ...
        # model_inputs["questions"] = questions
        return model_inputs
    ...
    with accelerator.main_process_first():
        ...
        eval_dataset = raw_datasets["validation"].map(
            partial(preprocess_summary_function,'validation'),
            batched=True,
            num_proc=args.preprocessing_num_workers,
            remove_columns=column_names,
            load_from_cache_file=not args.overwrite_cache,
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if args.with_tracking:
                    ...
                    accelerator.log({"train_loss": step_loss, "step": completed_steps,"lr":lr_scheduler.get_last_lr()[0]}, step=completed_steps)
