...
from prompt import llama_prompt,job_description,llama_fewshot_hotpot,llama_fewshot_nq,llama_fewshot_tqa,general_llama_prompt,job_description_con_long,job_description_con_short,llama_fewshot_musique,llama_fewshot_cot_long_musique,llama_fewshot_cot_short_musique
import json
import re
...
def arg_parse():
    ...
    parser.add_argument(
        "--use_cot_short",
        action="store_true",
        help="Whether to use CoT prompting short"
    )
    parser.add_argument(
        "--use_cot_long",
        action="store_true",
        help="Whether to use CoT prompting long"
    )
...
def main():
    ...
    globDesc = job_description
    if args.use_cot_short:
        fewshot_str = llama_fewshot_cot_short_musique
        globDesc = job_description_con_short
    elif args.use_cot_long:
        fewshot_str = llama_fewshot_cot_long_musique
        globDesc = job_description_con_long
    
    

        
    prompts = [
        general_llama_prompt.format(few_shot=fewshot_str,document=f"Passage: {example['Proposition'].removeprefix('<|start_header_id|>').removeprefix('assistant').removeprefix(':').strip()}",question=example['Question']) 
    ...
    if args.instruct == False:
        ...
        llm = LLM(model="Llama-3.1-8B-Instruct",tensor_parallel_size=args.parallel_size,gpu_memory_utilization=args.gpu_util)
        tokenizer = llm.get_tokenizer()
    ...
    if args.instruct == True:
        ...
        #con
        prompts = [tokenizer.apply_chat_template([{'role': 'system', 'content': globDesc.strip().removesuffix(":") + " Directly give the answer with no prefix or suffix:"},{'role': 'user', 'content': prompt}],tokenize=False,) for prompt in prompts]
    else:
